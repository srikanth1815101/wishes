(function(){"use strict";const n=window.SITE_CONFIG||{brandName:"CSRGO Wishes",brandDomain:"wishes.csrgo.com",whatsappNumber:"919999999999",currency:"₹",supabaseUrl:"",supabaseAnonKey:""};let f=[],o="All",i="",a=null,r="details",s=0,j=null;const t=document.getElementById("templates-grid"),g=document.getElementById("featured-row"),p=document.getElementById("featured-section"),y=document.getElementById("template-count"),O=document.getElementById("active-category-title"),d=document.getElementById("search-input"),l=document.getElementById("search-clear-btn"),m=document.getElementById("template-modal"),_=document.getElementById("modal-content-container"),h=document.getElementById("filter-sheet");document.addEventListener("DOMContentLoaded",()=>{A(),S(),F(),M()});async function M(){k();let e=null,s=null,t=window.FALLBACK_TEMPLATES;if(typeof t=="string")try{t=JSON.parse(t)}catch{t=null}if(n.supabaseUrl&&n.supabaseAnonKey&&window.supabase)try{const o=window.supabase.createClient(n.supabaseUrl,n.supabaseAnonKey),i=o.from("templates").select("*").eq("is_active",!0).order("sort_order",{ascending:!0}),a=new Promise(e=>setTimeout(()=>e({timeout:!0}),1500)),t=await Promise.race([i,a]);if(t&&!t.timeout){const{data:n,error:o}=t;o?s=o.message:n&&n.length>0&&(e=n)}}catch(e){console.warn("Supabase fetch failed, falling back to static data:",e)}if(!e)if(Array.isArray(t)&&t.length>0)e=t;else try{const t=await fetch("/data/templates.json");t.ok&&(e=await t.json())}catch(e){console.error("Error loading fallback JSON:",e)}e&&Array.isArray(e)?(f=e,c()):b(s||"Unable to load templates catalog.")}function H(){const e=i.trim().toLowerCase();return f.filter(t=>{const n=o==="All"||t.category===o;if(!n)return!1;if(!e)return!0;const s=[t.name,t.category,t.tagline||"",t.description||"",...t.tags||[]].join(" ").toLowerCase();return s.includes(e)})}function D(){return f.filter(e=>e.is_featured)}function c(){const t=D(),e=H();p&&g&&(t.length>0&&!i.trim()&&o==="All"?(p.style.display="block",z(t)):p.style.display="none"),O&&(O.textContent=o==="All"?"All Templates":o),y&&(y.textContent=`${e.length} design${e.length===1?"":"s"}`),e.length===0?T():C(e)}function z(t){g.innerHTML=t.map(t=>`
    ...
    `).join("")}function T(){if(!t)return;t.className="",t.innerHTML=`
    ...
    `;const e=document.getElementById("reset-filters-btn");e&&e.addEventListener("click",()=>{i="",o="All",d&&(d.value=""),v(),c()})}function b(n){if(!t)return;t.className="",t.innerHTML=`
    ...
    `}function x(e){const t=e.querySelectorAll(".template-select-btn");t.forEach(e=>{e.addEventListener("click",()=>{const n=e.getAttribute("data-id"),t=f.find(e=>e.id===n);t&&E(t)})})}function A(){if(!d)return;d.addEventListener("input",e=>{const t=e.target.value;l&&(l.style.display=t?"block":"none"),clearTimeout(j),j=setTimeout(()=>{i=t,c()},200)}),l&&l.addEventListener("click",()=>{d.value="",i="",l.style.display="none",c()})}function S(){const e=document.getElementById("category-bar");if(!e)return;e.addEventListener("click",e=>{const t=e.target.closest(".cat-btn");if(!t)return;o=t.getAttribute("data-category"),v(),c()})}function v(){const e=document.querySelectorAll(".cat-btn");e.forEach(e=>{const t=e.getAttribute("data-category");t===o?e.className="cat-btn shrink-0 rounded-full px-4 py-2 text-sm font-medium transition-all bg-ink-900 text-white shadow-card":e.className="cat-btn shrink-0 rounded-full px-4 py-2 text-sm font-medium transition-all border border-ink-200 bg-white text-ink-600 hover:border-ink-300 hover:text-ink-900"})}function F(){const e=document.querySelectorAll(".open-filter-btn");e.forEach(e=>{e.addEventListener("click",()=>{h&&(h.style.display="flex")})});const t=document.querySelectorAll(".close-filter-btn");t.forEach(e=>{e.addEventListener("click",()=>{h&&(h.style.display="none")})})}function E(e){a=e,r="details",s=0,document.body.style.overflow="hidden",u(),m&&(m.style.display="flex")}function w(){a=null,document.body.style.overflow="",m&&(m.style.display="none")}window.addEventListener("keydown",e=>{e.key==="Escape"&&a&&w()});function u(){if(!a||!_)return;const t=a,o=[t.image_url,...t.gallery||[]].filter(Boolean),i=Number(t.price).toLocaleString("en-IN");_.innerHTML=`
      <div class="relative z-10 flex max-h-[92vh] w-full max-w-2xl flex-col overflow-hidden rounded-t-3xl bg-white shadow-2xl animate-slide-up sm:rounded-3xl sm:animate-scale-in">
        ...
        <div class="relative aspect-[16/10] shrink-0 overflow-hidden bg-ink-100 sm:aspect-[16/8]">
          ${o[s]?`
            <img src="${e(o[s])}" alt="${e(t.name)}" class="h-full w-full object-cover" />
          `:""}
    ...
    `,R(t,o)}function N(t){const n=t.features||[];return`
