(function(){"use strict";const n=window.SITE_CONFIG||{brandName:"CSRGO Wishes",brandDomain:"wishes.csrgo.com",whatsappNumber:"919999999999",currency:"₹",supabaseUrl:"",supabaseAnonKey:""};let f=[],s="All",i="",a=null,r="details",o=0,j=null;const t=document.getElementById("templates-grid"),g=document.getElementById("featured-row"),p=document.getElementById("featured-section"),y=document.getElementById("template-count"),O=document.getElementById("active-category-title"),d=document.getElementById("search-input"),l=document.getElementById("search-clear-btn"),m=document.getElementById("template-modal"),_=document.getElementById("modal-content-container"),h=document.getElementById("filter-sheet");document.addEventListener("DOMContentLoaded",()=>{A(),S(),F(),M()});async function M(){k();let e=null,s=null,t=window.FALLBACK_TEMPLATES;if(typeof t=="string")try{t=JSON.parse(t)}catch{t=null}if(n.supabaseUrl&&n.supabaseAnonKey&&window.supabase)try{const o=window.supabase.createClient(n.supabaseUrl,n.supabaseAnonKey),i=o.from("templates").select("*").eq("is_active",!0).order("sort_order",{ascending:!0}),a=new Promise(e=>setTimeout(()=>e({timeout:!0}),1500)),t=await Promise.race([i,a]);if(t&&!t.timeout){const{data:n,error:o}=t;o?s=o.message:n&&n.length>0&&(e=n)}}catch(e){console.warn("Supabase fetch failed, falling back to static data:",e)}if(!e)if(Array.isArray(t)&&t.length>0)e=t;else try{const t=await fetch("/data/templates.json");t.ok&&(e=await t.json())}catch(e){console.error("Error loading fallback JSON:",e)}e&&Array.isArray(e)?(f=e,c()):b(s||"Unable to load templates catalog.")}function H(){const e=i.trim().toLowerCase();return f.filter(t=>{const n=s==="All"||t.category===s;if(!n)return!1;if(!e)return!0;const o=[t.name,t.category,t.tagline||"",t.description||"",...t.tags||[]].join(" ").toLowerCase();return o.includes(e)})}function D(){return f.filter(e=>e.is_featured)}function c(){const t=D(),e=H();p&&g&&(t.length>0&&!i.trim()&&s==="All"?(p.style.display="block",z(t)):p.style.display="none"),O&&(O.textContent=s==="All"?"All Templates":s),y&&(y.textContent=`${e.length} design${e.length===1?"":"s"}`),e.length===0?T():C(e)}function z(t){g.innerHTML=t.map(t=>`
      <button
        type="button"
        data-id="${t.id}"
    ...
    `).join(""),x(g)}function C(s){if(!t)return;t.className="mt-5 grid grid-cols-1 gap-5 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4",t.innerHTML=s.map((t,s)=>{const o=Math.min(s*60,400),i=(t.tags||[]).slice(0,3),a=Number(t.price).toLocaleString("en-IN");return`
        <button
          type="button"
          data-id="${t.id}"
          style="animation-delay: ${o}ms;"
      ...
      `}).join(""),x(t)}function k(){if(!t)return;t.className="mt-5 grid grid-cols-1 gap-5 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4",t.innerHTML=Array.from({length:8}).map(()=>`
    ...
    `).join("")}function T(){if(!t)return;const o=s==="All"?"":` for ${s}`;t.className="",t.innerHTML=`
    ...
    `;const n=document.getElementById("reset-filters-btn");n&&n.addEventListener("click",()=>{i="",s="All",d&&(d.value=""),v(),c()})}function b(n){if(!t)return;t.className="",t.innerHTML=`
    ...
    `}function x(e){const t=e.querySelectorAll(".template-select-btn");t.forEach(e=>{e.addEventListener("click",()=>{const n=e.getAttribute("data-id"),t=f.find(e=>e.id===n);t&&E(t)})})}function A(){if(!d)return;d.addEventListener("input",e=>{const t=e.target.value;l&&(l.style.display=t?"block":"none"),clearTimeout(j),j=setTimeout(()=>{i=t,c()},200)}),l&&l.addEventListener("click",()=>{d.value="",i="",l.style.display="none",c()})}function S(){const e=document.getElementById("category-bar");if(!e)return;e.addEventListener("click",e=>{const t=e.target.closest(".cat-btn");if(!t)return;s=t.getAttribute("data-category"),v(),c()})}function v(){const e=document.querySelectorAll(".cat-btn");e.forEach(e=>{const t=e.getAttribute("data-category");t===s?e.className="cat-btn shrink-0 rounded-full px-4 py-2 text-sm font-medium transition-all bg-ink-900 text-white shadow-card":e.className="cat-btn shrink-0 rounded-full px-4 py-2 text-sm font-medium transition-all border border-ink-200 bg-white text-ink-600 hover:border-ink-300 hover:text-ink-900"})}function F(){const e=document.querySelectorAll(".open-filter-btn");e.forEach(e=>{e.addEventListener("click",()=>{h&&(h.style.display="flex")})});const t=document.querySelectorAll(".close-filter-btn");t.forEach(e=>{e.addEventListener("click",()=>{h&&(h.style.display="none")})})}function E(e){a=e,r="details",o=0,document.body.style.overflow="hidden",u(),m&&(m.style.display="flex")}function w(){a=null,document.body.style.overflow="",m&&(m.style.display="none")}window.addEventListener("keydown",e=>{e.key==="Escape"&&a&&w()});function u(){if(!a||!_)return;const t=a,s=[t.image_url,...t.gallery||[]].filter(Boolean),i=Number(t.price).toLocaleString("en-IN");_.innerHTML=`
      <div class="relative z-10 flex max-h-[92vh] w-full max-w-2xl flex-col overflow-hidden rounded-t-3xl bg-white shadow-2xl animate-slide-up sm:rounded-3xl sm:animate-scale-in">
        ...
        <div class="relative aspect-[16/10] shrink-0 overflow-hidden bg-ink-100 sm:aspect-[16/8]">
          ${s[o]?`
            <img src="${e(s[o])}" alt="${e(t.name)}" class="h-full w-full object-cover" />
          `:""}
          ...
          ${s.length>1?`
            ...
            <div class="absolute bottom-3 left-1/2 flex -translate-x-1/2 gap-1.5">
              ${s.map((e,t)=>`
                <span class="h-1.5 rounded-full transition-all ${t===o?"w-5 bg-white":"w-1.5 bg-white/50"}"></span>
              `).join("")}
            </div>
          `:""}
    ...
    `,R(t,s)}function N(t){const n=t.features||[];return`
    ...
    `}function R(e,t){const n=document.getElementById("modal-close-btn");n&&n.addEventListener("click",w);const s=document.getElementById("gallery-prev-btn");s&&s.addEventListener("click",()=>{o=(o-1+t.length)%t.length,u()});const i=document.getElementById("gallery-next-btn");i&&i.addEventListener("click",()=>{o=(o+1)%t.length,u()});const a=document.getElementById("modal-order-step-btn");a&&a.addEventListener("click",()=>{r="order",u()});const c=document.getElementById("modal-back-step-btn");c&&c.addEventListener("click",()=>{r="details",u()});const l=document.getElementById("modal-submit-whatsapp-btn");l&&l.addEventListener("click",()=>{P(e)})}function P(e){const l=document.getElementById("field-recipient"),h=document.getElementById("field-date"),u=document.getElementById("field-yourname"),g=document.getElementById("field-phone"),d=document.getElementById("field-email"),f=document.getElementById("field-notes"),t=document.getElementById("err-recipient"),a=document.getElementById("err-yourname"),i=document.getElementById("err-phone"),o=document.getElementById("err-email");let s=!0;t&&t.classList.add("hidden"),a&&a.classList.add("hidden"),i&&i.classList.add("hidden"),o&&o.classList.add("hidden");const m=l?l.value.trim():"",v=h?h.value:"",p=u?u.value.trim():"",c=g?g.value.trim():"",r=d?d.value.trim():"",b=f?f.value.trim():"";if(m||(t&&(t.textContent="Please enter the recipient name",t.classList.remove("hidden")),s=!1),p||(a&&(a.textContent="Please enter your name",a.classList.remove("hidden")),s=!1),(!c||c.replace(/\D/g,"").length<8)&&(i&&(i.textContent="Please enter a valid phone number",i.classList.remove("hidden")),s=!1),r&&!/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(r)&&(o&&(o.textContent="Please enter a valid email",o.classList.remove("hidden")),s=!1),!s)return;const j=Number(e.price).toLocaleString("en-IN"),y=[`*New Order — ${n.brandDomain}*`,``,`*Template:* ${e.name}`,`*Category:* ${e.category}`,`*Price:* ${n.currency}${j}`,``,`*Recipient / Occasion Name:* ${m}`,`*Occasion Date:* ${v||"Not specified"}`,`*Your Name:* ${p}`,`*Phone:* ${c}`,`*Email:* ${r||"Not provided"}`,``,`*Notes:*`,b||"None"],_=encodeURIComponent(y.join(`
`)),w=`https://wa.me/${n.whatsappNumber}?text=${_}`;window.open(w,"_blank","noopener,noreferrer")}function e(e){return typeof e!="string"?"":e.replace(/&/g,"&amp;").replace(/</g,"&lt;").replace(/>/g,"&gt;").replace(/"/g,"&quot;").replace(/'/g,"&#039;")}})()