(function(){"use strict";const o=window.SITE_CONFIG||{brandName:"CSRGO Wishes",brandDomain:"wishes.csrgo.com",whatsappNumber:"919999999999",currency:"₹",supabaseUrl:"",supabaseAnonKey:""};let u=[],s="All",r="",a=null,i="details",n=0,j=null;const t=document.getElementById("templates-grid"),f=document.getElementById("featured-row"),p=document.getElementById("featured-section"),w=document.getElementById("template-count"),x=document.getElementById("active-category-title"),c=document.getElementById("search-input"),l=document.getElementById("search-clear-btn"),m=document.getElementById("template-modal"),_=document.getElementById("modal-content-container"),h=document.getElementById("filter-sheet");document.addEventListener("DOMContentLoaded",()=>{A(),S(),F(),z()});async function z(){M();let t=null,e=window.FALLBACK_TEMPLATES;if(typeof e=="string")try{e=JSON.parse(e)}catch{e=null}if(Array.isArray(e)&&e.length>0)t=e;else try{const e=await fetch("/data/templates.json");e.ok&&(t=await e.json())}catch(e){console.error("Error loading templates JSON:",e)}t&&Array.isArray(t)?(u=t,d()):T("Failed to load templates.")}function I(){const e=r.trim().toLowerCase();return u.filter(t=>{const n=s==="All"||t.category===s;if(!n)return!1;if(!e)return!0;const o=[t.name,t.category,t.tagline||"",t.description||"",...t.tags||[]].join(" ").toLowerCase();return o.includes(e)})}function R(){return u.filter(e=>e.is_featured)}function d(){const t=R(),e=I();p&&f&&(t.length>0&&!r.trim()&&s==="All"?(p.style.display="block",D(t)):p.style.display="none"),x&&(x.textContent=s==="All"?"All Templates":s),w&&(w.textContent=`${e.length} design${e.length===1?"":"s"}`),e.length===0?k():C(e)}function D(t){f.innerHTML=t.map(t=>`
      <button
        type="button"
        data-id="${t.id}"
    ...
    `).join(""),b(f)}function C(n){if(!t)return;t.className="mt-5 grid grid-cols-1 gap-5 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4",t.innerHTML=n.map((t,n)=>{const s=Math.min(n*60,400),i=(t.tags||[]).slice(0,3),a=Number(t.price).toLocaleString("en-IN");return`
        <button
          type="button"
          data-id="${t.id}"
      ...
      `}).join(""),b(t)}function M(){if(!t)return;t.className="mt-5 grid grid-cols-1 gap-5 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4",t.innerHTML=Array.from({length:8}).map(()=>`
    ...
    `).join("")}function k(){if(!t)return;const o=s==="All"?"":` for ${s}`;t.className="",t.innerHTML=`
    ...
    `;const n=document.getElementById("reset-filters-btn");n&&n.addEventListener("click",()=>{r="",s="All",c&&(c.value=""),v(),d()})}function T(n){if(!t)return;t.className="",t.innerHTML=`
    ...
    `}function b(e){const t=e.querySelectorAll(".template-select-btn");t.forEach(e=>{e.addEventListener("click",()=>{const n=e.getAttribute("data-id"),t=u.find(e=>e.id===n);t&&E(t)})})}function A(){if(!c)return;c.addEventListener("input",e=>{const t=e.target.value;l&&(l.style.display=t?"block":"none"),clearTimeout(j),j=setTimeout(()=>{r=t,d()},200)}),l&&l.addEventListener("click",()=>{c.value="",r="",l.style.display="none",d()})}function S(){const e=document.getElementById("category-bar");if(!e)return;e.addEventListener("click",e=>{const t=e.target.closest(".cat-btn");if(!t)return;s=t.getAttribute("data-category"),v(),d()})}function v(){const e=document.querySelectorAll(".cat-btn");e.forEach(e=>{const t=e.getAttribute("data-category");t===s?e.className="cat-btn shrink-0 rounded-full px-4 py-2 text-sm font-medium transition-all bg-ink-900 text-white shadow-card":e.className="cat-btn shrink-0 rounded-full px-4 py-2 text-sm font-medium transition-all border border-ink-200 bg-white text-ink-600 hover:border-ink-300 hover:text-ink-900"})}function F(){const e=document.querySelectorAll(".open-filter-btn");e.forEach(e=>{e.addEventListener("click",()=>{h&&(h.style.display="flex")})});const t=document.querySelectorAll(".close-filter-btn");t.forEach(e=>{e.addEventListener("click",()=>{h&&(h.style.display="none")})})}function E(e){a=e,i="details",n=0,document.body.style.overflow="hidden",g(),m&&(m.style.display="flex")}function O(){a=null,document.body.style.overflow="",m&&(m.style.display="none")}window.addEventListener("keydown",e=>{e.key==="Escape"&&a&&O()});function g(){if(!a||!_)return;const t=a,r=t.gallery&&t.gallery.length>0?t.gallery:[t.image_url],s=Array.from(new Set([t.image_url,...r])).filter(Boolean),c=Number(t.price).toLocaleString("en-IN");_.innerHTML=`
    ...
    `,P(t,s)}function N(t){const n=t.features||[];return`
