(function(){"use strict";const c=window.SITE_CONFIG||{brandName:"CSRGO Wishes",brandDomain:"wishes.csrgo.com",whatsappNumber:"919999999999",currency:"₹",supabaseUrl:"",supabaseAnonKey:""};let m=[],n="All",a="",o=null,i="details",s=0,j=null;const t=document.getElementById("templates-grid"),p=document.getElementById("featured-row"),g=document.getElementById("featured-section"),y=document.getElementById("template-count"),O=document.getElementById("active-category-title"),l=document.getElementById("search-input"),d=document.getElementById("search-clear-btn"),f=document.getElementById("template-modal"),_=document.getElementById("modal-content-container"),h=document.getElementById("filter-sheet");document.addEventListener("DOMContentLoaded",()=>{A(),S(),C(),M()});async function M(){k();let t=null,e=window.FALLBACK_TEMPLATES;if(typeof e=="string")try{e=JSON.parse(e)}catch{e=null}if(Array.isArray(e)&&e.length>0)t=e;else try{const e=await fetch("/data/templates.json");e.ok&&(t=await e.json())}catch(e){console.error("Error loading templates JSON:",e)}t&&Array.isArray(t)?(m=t,u()):E("Failed to load templates.")}function H(){const e=a.trim().toLowerCase();return m.filter(t=>{const s=n==="All"||t.category===n;if(!s)return!1;if(!e)return!0;const o=[t.name,t.category,t.tagline||"",t.description||"",...t.tags||[]].join(" ").toLowerCase();return o.includes(e)})}function D(){return m.filter(e=>e.is_featured)}function u(){const t=D(),e=H();g&&p&&(t.length>0&&!a.trim()&&n==="All"?(g.style.display="block",z(t)):g.style.display="none"),O&&(O.textContent=n==="All"?"All Templates":n),y&&(y.textContent=`${e.length} design${e.length===1?"":"s"}`),e.length===0?F():x(e)}function z(t){p.innerHTML=t.map(t=>`
      <button
        type="button"
        data-id="${t.id}"
    ...
    `).join(""),b(p)}function x(n){if(!t)return;t.className="mt-5 grid grid-cols-1 gap-5 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4",t.innerHTML=n.map((t,n)=>{const s=Math.min(n*60,400),o=(t.tags||[]).slice(0,3);return`
        <button
          type="button"
          data-id="${t.id}"
    ...
    `;const s=document.getElementById("reset-filters-btn");s&&s.addEventListener("click",()=>{a="",n="All",l&&(l.value=""),v(),u()})}function E(n){if(!t)return;t.className="",t.innerHTML=`
    ...
    `}function b(e){const t=e.querySelectorAll(".template-select-btn");t.forEach(e=>{e.addEventListener("click",()=>{const n=e.getAttribute("data-id"),t=m.find(e=>e.id===n);t&&T(t)})})}function A(){if(!l)return;l.addEventListener("input",e=>{const t=e.target.value;d&&(d.style.display=t?"block":"none"),clearTimeout(j),j=setTimeout(()=>{a=t,u()},200)}),d&&d.addEventListener("click",()=>{l.value="",a="",d.style.display="none",u()})}function S(){const e=document.getElementById("category-bar");if(!e)return;e.addEventListener("click",e=>{const t=e.target.closest(".cat-btn");if(!t)return;n=t.getAttribute("data-category"),v(),u()})}function v(){const e=document.querySelectorAll(".cat-btn");e.forEach(e=>{const t=e.getAttribute("data-category");t===n?e.className="cat-btn shrink-0 rounded-full px-4 py-2 text-sm font-medium transition-all bg-ink-900 text-white shadow-card":e.className="cat-btn shrink-0 rounded-full px-4 py-2 text-sm font-medium transition-all border border-ink-200 bg-white text-ink-600 hover:border-ink-300 hover:text-ink-900"})}function C(){const e=document.querySelectorAll(".open-filter-btn");e.forEach(e=>{e.addEventListener("click",()=>{h&&(h.style.display="flex")})});const t=document.querySelectorAll(".close-filter-btn");t.forEach(e=>{e.addEventListener("click",()=>{h&&(h.style.display="none")})})}function T(e){o=e,i="details",s=0,document.body.style.overflow="hidden",r(),f&&(f.style.display="flex")}function w(){o=null,document.body.style.overflow="",f&&(f.style.display="none")}window.addEventListener("keydown",e=>{e.key==="Escape"&&o&&w()});function r(){if(!o||!_)return;const t=o,n=[t.image_url,...t.gallery||[]].filter(Boolean),a=Number(t.price).toLocaleString("en-IN");_.innerHTML=`
